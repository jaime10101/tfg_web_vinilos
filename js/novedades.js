...
/* ============================================================
   DATOS
   TODO (Spring Boot): GET /api/productos?orden=reciente
   ============================================================ */
const PRODUCTS = [
...
function renderGrid() {
    ...
    const visible = productosFiltrados.slice(0, state.itemsToShow);

    /* Sin resultados */
    if (visible.length === 0) {
        grid.innerHTML = `<div style="grid-column:1/-1;text-align:center;padding:60px 20px;">
            <p style="font-size:18px;color:var(--apagado);">No se encontraron productos</p>
        </div>`;
        actualizarCargarMas();
        return;
    }
    ...
    grid.innerHTML = visible.map(p => {
        /* Badge opcional — insignia superior izquierda */
        const badgeHTML = p.badge
            ? `<span class="insignia-producto ${p.badgeType}">${p.badge}</span>`
            : '';
        ...
        const imgHTML = p.image
            ...
            : `<div class="icono-placeholder"><i class="fas fa-music"></i></div>`;
        ...
        <div class="tarjeta-producto" data-id="${p.id}" data-category="${p.category}">
            <div class="imagen-producto">
                ...
                <div class="capa-hover">
                    <a href="${destino}" class="btn-accion">
    ...
    /* Clic en tarjeta — navega a la página de detalle correspondiente */
...
/* Muestra u oculta el botón cargar más según productos restantes */
function actualizarCargarMas() {
    const zona   = document.querySelector('.zona-cargar-mas');
    const btn    = document.getElementById('btnCargarMas');
    const quedan = productosFiltrados.length - state.itemsToShow;

    if (quedan <= 0) {
        zona.style.display = 'none';
    } else {
        zona.style.display = 'block';
        btn.innerHTML = `Cargar Más Novedades (${quedan} restantes) <i class="fas fa-chevron-down"></i>`;
    }
}
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    /* Filtros por categoría — botones Todos, Vinilos, Merch, Posters */
    document.querySelectorAll('.btn-filtro').forEach(btn => {
        btn.addEventListener('click', function () {
            if (this.tagName === 'SELECT') return;
            document.querySelectorAll('.btn-filtro').forEach(b => {
                if (b.tagName !== 'SELECT') b.classList.remove('activo');
            });
    ...
    /* Newsletter — limpiar input al suscribir */
    document.getElementById('btnSuscribir').addEventListener('click', () => {
        const email = document.getElementById('inputEmail').value;
        if (email) {
            document.getElementById('inputEmail').value = '';
            /* TODO (Spring Boot): POST /api/newsletter { email } */
        }
    });
