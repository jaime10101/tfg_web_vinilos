/* ============================================================
   PLAYLIST.JS — Lógica del reproductor de playlist
...
/* ============================================================
   ...
   TODO (Supabase): reemplazar por:
     const { data } = await supabase.from('playlist').select('*').order('orden');
   ============================================================ */
const CANCIONES = [
...
function cargarCancion(index) {
    ...
    if (filaActiva) { filaActiva.classList.add('activa'); filaActiva.scrollIntoView({ block: 'nearest', behavior: 'smooth' }); }
}
...
function cancionSiguiente() {
    let siguiente;
    if (modoAleatorio) {
        do { siguiente = Math.floor(Math.random() * CANCIONES.length); } while (siguiente === indiceActual && CANCIONES.length > 1);
    } else {
...
audio.addEventListener('ended', () => { modoRepetir ? (audio.currentTime = 0, reproducir()) : cancionSiguiente(); });
...
sliderVolumen.addEventListener('input', () => {
    audio.volume = parseFloat(sliderVolumen.value);
    iconoVolumen.className = audio.volume === 0 ? 'fa-solid fa-volume-xmark' : audio.volume < 0.5 ? 'fa-solid fa-volume-low' : 'fa-solid fa-volume-high';
});
iconoVolumen.addEventListener('click', () => {
    if (audio.volume > 0) { iconoVolumen.dataset.prevVol = audio.volume; audio.volume = 0; sliderVolumen.value = 0; iconoVolumen.className = 'fa-solid fa-volume-xmark'; }
    else { const prev = parseFloat(iconoVolumen.dataset.prevVol || 0.8); audio.volume = prev; sliderVolumen.value = prev; iconoVolumen.className = 'fa-solid fa-volume-high'; }
});
...
btnAleatorio.addEventListener('click', () => { modoAleatorio = !modoAleatorio; btnAleatorio.classList.toggle('activo', modoAleatorio); });
btnRepetir.addEventListener('click',   () => { modoRepetir   = !modoRepetir;   btnRepetir.classList.toggle('activo', modoRepetir); });
