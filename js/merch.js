/* ============================================================
   MERCH.JS — Lógica de la página de merchandising
...
let state = { tab: 'all', categories: [], artists: [], maxPrice: 100, query: '', sort: 'popular', page: 1 };
...
function getFiltered() {
    ...
    if (state.categories.length > 0 && !state.categories.includes('all')) result = result.filter(p => state.categories.includes(p.category));
    ...
    if (state.query) result = result.filter(p => p.name.toLowerCase().includes(state.query) || p.artist.toLowerCase().includes(state.query));
...
function renderGrid(items) {
    ...
    if (items.length === 0) {
        grid.innerHTML = `<div style="grid-column:1/-1;text-align:center;padding:60px 20px;color:var(--apagado);"><p style="font-size:18px;">No se encontraron productos</p></div>`;
        return;
    }
    grid.innerHTML = items.map(p => {
        const discountPct = p.oldPrice ? Math.round(((p.oldPrice - p.price) / p.oldPrice) * 100) : null;
        const badgeHTML   = p.badge === 'new' ? `<span class="insignia-producto nuevo">NUEVO</span>` : p.badge === 'discount' ? `<span class="insignia-producto descuento">-${discountPct}%</span>` : '';
        const destino     = p.category === 'posters' ? `detalle/detalle.html?id=${p.id}` : `detalle/detalle_merch.html?id=${p.id}`;
        return `
    ...
    grid.querySelectorAll('.tarjeta-producto').forEach(card => {
        card.addEventListener('click', e => {
            ...
            window.location.href = cat === 'posters' ? `detalle/detalle.html?id=${id}` : `detalle/detalle_merch.html?id=${id}`;
        });
    });
}
...
function renderPagination(total) {
    ...
    html += `<button class="btn-pagina" id="btnSiguiente" ${state.page === totalPages ? 'disabled' : ''}>›</button>`;
    container.innerHTML = html;
    document.getElementById('btnAnterior').addEventListener('click', () => { if (state.page > 1) { state.page--; update(); } });
    document.getElementById('btnSiguiente').addEventListener('click', () => { if (state.page < totalPages) { state.page++; update(); } });
    container.querySelectorAll('.btn-pagina[data-page]').forEach(btn => {
        btn.addEventListener('click', () => { state.page = parseInt(btn.dataset.page); update(); });
    });
}
...
function update() {
    ...
    renderGrid(filteredProducts.slice(start, start + ITEMS_PER_PAGE));
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    document.getElementById('cat-all').addEventListener('change', function () {
        if (this.checked) { document.querySelectorAll('[id^="cat-"]:not(#cat-all)').forEach(cb => cb.checked = false); state.categories = []; }
        state.page = 1; update();
    });
    ...
    document.querySelectorAll('[id^="cat-"]:not(#cat-all)').forEach(cb => {
        cb.addEventListener('change', function () {
            ...
            if (this.checked) { document.getElementById('cat-all').checked = false; state.categories.push(cat); }
            else { state.categories = state.categories.filter(c => c !== cat); if (state.categories.length === 0) document.getElementById('cat-all').checked = true; }
            state.page = 1; update();
        });
    });

    const artistMap = { 'art-rosalia': 'Rosalía', 'art-arctic': 'Arctic Monkeys', 'art-taylor': 'Taylor Swift', 'art-bad': 'Bad Bunny', 'art-weeknd': 'The Weeknd' };
    document.querySelectorAll('[id^="art-"]').forEach(cb => {
        cb.addEventListener('change', function () {
            const artist = artistMap[this.id];
            state.artists = this.checked ? [...state.artists, artist] : state.artists.filter(a => a !== artist);
            state.page = 1; update();
        });
    });
    ...
    document.getElementById('sliderPrecio').addEventListener('input', function () {
        ...
        document.getElementById('precioMax').textContent = `${state.maxPrice}€${state.maxPrice >= 100 ? '+' : ''}`;
        state.page = 1; update();
    });

    document.getElementById('selectorOrden').addEventListener('change', function () { state.sort = this.value; state.page = 1; update(); });
    document.getElementById('campoBusqueda').addEventListener('input', function () { state.query = this.value.toLowerCase(); state.page = 1; update(); });
