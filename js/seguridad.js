/* ============================================================
   ...
   Toda la configuración sensible va en:

   · Spring Boot  → src/main/resources/application.properties
                    (o variables de entorno del servidor)
   · Frontend     → config.js  (NO subir a GitHub → .gitignore)

   CÓMO USAR config.js:
   Crea el archivo /js/config.js con este contenido:
   ─────────────────────────────────────────────────
   window.VS_CONFIG = {
     apiUrl:       'http://localhost:8080/api',  // dev
     supabaseUrl:  'https://xxxx.supabase.co',
     supabaseKey:  'eyJ...'                      // anon key (pública)
   };
   ─────────────────────────────────────────────────
   En producción cambia apiUrl a tu dominio real.
   El supabaseKey es el anon key — es seguro en el frontend.
   NUNCA pongas el service_role key aquí.
   ============================================================ */

/* Lee la config desde config.js — fallback a localhost para desarrollo */
const API_BASE_URL    = window.VS_CONFIG?.apiUrl      || 'http://localhost:8080/api';
const SUPABASE_URL    = window.VS_CONFIG?.supabaseUrl || '';
const SUPABASE_ANON   = window.VS_CONFIG?.supabaseKey || '';
